...
export default function Jobs() {
  ...
  const loadJobs = async () => {
    ...
    try {
      ...
      // Candidates should see only active jobs by default
      if (!isAdmin && !status) params.append('status', 'active')
  ...
  const handleArchive = async (job, archive = true) => {
    try {
      ...
      if (!response.ok) {
        ...
        alert(`Failed to archive job: ${errorData.error || 'Unknown error'}`)
        return
      }
  ...
  const handleDragEnd = async (result) => {
    ...
    try {
      await fetch(`/api/jobs/${removed.id}/reorder`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          fromOrder: removed.order,
